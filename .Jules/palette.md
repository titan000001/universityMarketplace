...
## 2025-05-18 - Loading States for Async Actions
**Learning:** Users lack feedback during async authentication requests (login/register) and other actions, leading to potential double-submissions and uncertainty.
**Action:** Implemented a reusable `setLoading` utility (`public/js/utils/loading.js`) that toggles button state and displays a spinner. This pattern should be applied to all future async form submissions to maintain consistency.
...
## 2026-02-14 - Standardizing UX Utilities
**Learning:** Inconsistent implementation of common patterns (like loading states) across different views leads to maintenance fragility and accessibility gaps (e.g., missing `aria-busy`). Manual implementations are prone to errors and code duplication.
**Action:** Rigorously enforce the use of shared utilities like `setLoading` for all async actions. Refactored checkout flow to use the standard utility, ensuring consistent visual and assistive technology feedback.
